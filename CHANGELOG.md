...
- Bump locked `esbuild`, `vite`, and `hono` dependency versions to resolve
  active security advisories for the development toolchain and MCP runtime.

## [0.8.1] - 2026-06-05

### Fixed

- Bump the locked transitive `hono` runtime dependency to `4.12.23`,
  resolving Dependabot alerts GHSA-f577-qrjj-4474, GHSA-2gcr-mfcq-wcc3,
  GHSA-xrhx-7g5j-rcj5, and GHSA-3hrh-pfw6-9m5x.
...
- Add Object Tree metadata consistency checks for behavior source facts, MCP
  control-reference behavior parity, metadata exclusivity, and write-tested
...
- Refresh manual, MCP, and VSIX smoke-test install examples so packaged docs no
  longer point at stale release versions.
...
- Resolve Beam color palette boundary behavior using checked-in mixed
