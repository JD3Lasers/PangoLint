...
| `src/runtime/` | BEYOND communication and runtime commands | Talk transport, OSC, command batch sending, readback workflows, runtime configuration, object value assignment, validation commands, and VS Code runtime adapters | Keep runtime safety explicit and readback-first by default. |
