...
Runs against a freshly built `pangolint-0.4.5.vsix`. The goal is to confirm
...
`pangolint-0.4.5.vsix` and `pangolint.vsix.sha256` into the repo root. Verify
...
```powershell
code --install-extension .\pangolint-0.4.5.vsix --force
```
...
3. Pick `pangolint-0.4.5.vsix`.
4. Reload the window when prompted.

### Verify install

- Extensions view shows **PangoLint 0.4.5** (publisher `jd3`).
